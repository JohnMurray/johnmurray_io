...
Above we define several functions, meant to be called in order. Each function returns a possible
error and the next functio takes the success value of the previous function. The code starting 
with `for {` is some syntactic sugar that allows us to call `map` and `flatMap` which are
essentially ways of calling the next function assuming the previous one succeeded. If there
was an error, then the value of `model` will be a failed `Try` (`Failure` in Scala) with the
error.

This allows the programmer to write code that describes the "happy path", dealing with any
errors at the end. Some argue this style is more readable. ¯\\\_(ツ)_/¯

I mean, it has it's own
...
to create/use the `Try`. Let's define some basic functions for creating an interacting with
`Try`:
...
error handling. For this we have to break out the big guns, `map` and `flatMap`. If you haven't
heard of these functions yet, maybe go try googling "higher order functions" to get a better
idea. I'll avoid explaining them here since that could be a whole other post on it's own.
...
that is working with `interface {}` which is obiously the superior way of writing code in
Go:
...
maintainable code over immediate error checking?

### Less Horrible?

Even though so far we have a working `Try` complete with `interface {}` everywhere (a favorite
style of _"real programmers"_), the benevolent overlords of Go built us `go generate` and it would
be a shame to not find a way to shove that into this blog post.

Thanks to [genny][github_genny], onwards and upwards my friends!

First a template without any of the tranformation functions (`Map`, `FlatMap`, etc):
...
With this + `genny`, we can generate some basic `Try` code for a specific type without any compsition
...
Of course, without being able to compose things with `Try`s of other type, this isn't that great. So let's
...
Yay! This is all very horrible, but it actually works in a usable way. Feel free to spread the _love_ ;-)
